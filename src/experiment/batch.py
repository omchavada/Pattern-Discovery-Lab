...
class BatchDownloader:
    ...
    def __init__(self, manager: ExperimentManager | None = None, max_workers: int = 10) -> None:
    ...
    def _run_single_safely(
        self, name: str, ticker: str, start: str, end: str, workspace: str
    ) -> ExperimentContext:
        """Wrapper to ensure an error on one ticker does not crash the entire batch."""
        try:
            return self.manager.run_experiment(
                name=name, ticker=ticker, start=start, end=end, workspace=workspace
            )
    ...
    def execute_batch(
        self, tickers: list[str], start: str, end: str, workspace: str
        ...
        logger.info(
            f"Launching batch execution for {total_tickers} tickers in workspace '{workspace}'..."
        )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(future_to_ticker):
                ...
                try:
                    ...
                    contexts.append(ctx)
                    logger.info(
                        f"Batch progress: {len(contexts)}/{total_tickers} finalized ({ticker})"
                    )
                except Exception as e:
