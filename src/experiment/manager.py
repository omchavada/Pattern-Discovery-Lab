...
class ExperimentManager:
    ...
    def run_experiment(self, name: str, ticker: str, start: str, end: str, workspace: str = "default", tags: Dict[str, str] = None, force_refresh: bool = False) -> ExperimentContext:
        """
        ...
        if not force_refresh:
            ...
            if not cached_result.empty:
                ...
                logger.info(f"CACHE HIT: Pristine data found for {ticker} in {workspace}/{exp_id}. Bypassing pipeline.")
                
                # Rehydrate Context from cache
                ctx = ExperimentContext(
                    workspace=workspace, experiment_id=exp_id, name=name, 
                    ticker=ticker, source="Yahoo", start_date=start, end_date=end, 
                    state=ExperimentState.COMPLETED
                )
                
                # Load the Parquet file back into memory for downstream Feature Engineering
                exp_dir = self.tracker._get_exp_dir(workspace, exp_id)
                ctx.market_data = pd.read_parquet(exp_dir / "market_data.parquet")
                return ctx
