...
class YahooStandardizer(BaseStandardizer):
    ...
    def standardize(self, df: pd.DataFrame, ticker: Ticker) -> MarketData:
        ...
        standard_df.columns = [str(col).lower().strip().replace(" ", "_") for col in standard_df.columns]
        
        # 4. Extract and Canonicalize Date Index
        if 'date' not in standard_df.columns and standard_df.index.name and standard_df.index.name.lower() == 'date':
